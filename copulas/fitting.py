...
from .distributions import NormalDist, KDEDist
...
class CopulaSelection:
    def __init__(
        ...
        self.marginal_type = marginal_type
        self.alpha = alpha
        self.candidate_copulas = ["gumbel", "clayton", "independence", "normal"]
    ...
    def optimize_mixed_copula(self, mixed_copula, bnds=(0, 1)):
        opt_func = lambda alpha: -self.log_likelihood(mixed_copula.alpha_update(alpha))
        alpha = optimize.minimize_scalar(opt_func, bounds=bnds, method="bounded").x
    ...
    def evaluate_mixed_copula(self):
        ...
        mixed_copulae = [cop for cop in mixed_copulae]
        none_ = []
        res = []
