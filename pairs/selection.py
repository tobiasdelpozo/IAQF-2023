...
class PairSelection:
    ...
    def get_pairs(self, number_of_pairs=None):
        ...
        for s1, s2 in self.coint_result[["sym1", "sym2"]].values:
            if len(final_pairs) >= number_of_pairs:
                break
            if s1 not in selected_tickers and s2 not in selected_tickers:
...
"""
IDEAL SETUP FOR BACKTESTER.
pairs = PairSelection(price).get_pairs()
theta, copula = CopulaSelection(price, pairs).get_params()
hedge_ratio = HedgeRatioSelection(price, pairs, market).get_hedge_ratio()
"""
